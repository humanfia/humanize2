...
class SessionBase(ABC):
    ...
    def interrupt(self, *, why: str) -> None:
        ...
        Args:
            why: What to tell the backend, and whoever is watching, about why.
        """
        raise NotImplementedError
...
- `interrupt` MUST stop the turn under way without ending the conversation, and MUST say why:
  a turn that ended without a reason reads as a turn that crashed. The turn it stops fails;
  the session it stops it in MUST still be there, under the id it was opened with, for the
  next turn to resume. A backend with nothing listening to be told MUST raise
  `NotImplementedError`, and whatever asked MUST go on to what it does about a turn that
  cannot be asked rather than treating that as a failure of the turn.
