...
- `AgentDefaults` MUST be what a flow writes beside a place to say them, the way `Goal`,
  `Remote` and `Isolated` are written, and a place that writes none MUST run at what it
  declares by default -- `bypass`, goals on, the web readable -- which is what every agent of
  every flow ran at before a flow could say. It MUST refuse a rung no backend has a word for
  where it is written, so that a flow declaring one is refused as the flow is read rather than
