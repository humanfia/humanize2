...
- `hmz anchor` MUST NOT be a command here: it is not a thing to do to a flow that is running,
  ...
  be one either -- it is a thing done to a run that has already happened, so it comes of
  exporting a run gone into from `/epics` rather than being a command of its own.
...
- What is written MUST be everything the run wrote down and everything its sessions were
  logged to, and MUST NOT hold the screen -- what is on the screen is not that run, which may
