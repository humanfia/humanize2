...
An **atlas** is the other bargain a weaver may strike. Its small declarative body is read
before its first node runs and compiled into a **prophecy** — a typed, canonical graph of every
node, edge, branch, loop and way out.
...
other piece of work belong in a node, where the whole language is available again:
...
The ordinary [flow](/features/flows) remains the better choice when the shape of the work is
meant to emerge while it runs.
...
A loop becomes an explicit back-edge, its head run again with the values the body changed. If
the body changes nothing the head reads, the loop would make the same decision forever and is
refused. A return becomes a named way out of the graph, so a supernode answers with what the
...
The same zero-execution reading checks ordinary node code and regular flows, establishing
...
It does not pretend to prove that an exit is reachable, follow a value through arbitrary calls,
or predict a model. Those questions belong to tests that drive the flow with stand-in agents
and adversarial answers. Neither half needs a real model.

## One graph has one identity

A prophecy has a canonical text: nodes, edges, shapes and nested prophecies are ordered by what
they are, not by source formatting. Its identity is the first sixteen hexadecimal characters of
that text's SHA-256 digest. Reflowing a docstring, adding a comment or changing a node's
implementation therefore does not invent a new graph; changing a call site, an edge, a shape or
a nested prophecy does. That is what lets the work under a stable graph evolve without
pretending a changed graph is the old one.
...
held; code is not swapped underneath a graph halfway through a round. A regular flow does the
opposite — its handle keeps a name and reads the target, including the modules beside it, again
at each call, and a running flow may rewrite that target. An atlas gives up that within-run hot
reload so its prophecy has no holes. Its files are read again by the next run.

## Picking up is another walk over the same graph

Every completed visit writes its answer under both the node id and the visit number, so the
third visit to one node cannot overwrite the first two. Visits inside a supernode are kept
beneath the outer visit, however deep the nesting goes.

A run picked up starts at the graph's way in and rebuilds the values already written down.
Completed visits return their kept answers without running, and the first visit with no answer
is exactly where work continues. By default a node interrupted partway runs again; one that has
certainly taken effect may instead declare that it should be skipped on resume, but it must
answer with nothing so no missing value is hidden.

This is precise workflow recovery, not process recovery — backend conversations do not come
back. The saved state carries the prophecy digest, so a changed graph clears the old visits and
starts at the top. A changed node body leaves the digest matching, the completed visits
completed, and the first unfinished visit running that node's current code.
