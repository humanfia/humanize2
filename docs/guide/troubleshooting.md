...
The file holds [several flows](/reference/flows.md#several-flows-in-one-file) and none of them under its
...
Look at whether it is still [pinned above the prompt](/reference/tui.md#talking-to-a-running-flow): a line
...
the turn says it landed. See [Remote execution](/reference/remote-execution.md#anchoring-a-flow).
...
See [what each backend can do](/reference/agents.md#what-each-backend-can-do).
...
Those four, and nothing else. See [Targets](/reference/remote-execution.md#targets).
...
The full list is [What is not guaranteed](/reference/remote-execution.md#what-is-not-guaranteed).
...
- The `SPEC.md` beside the code says what it is *supposed* to do, normatively —
  ...
  strangely.
- [Architecture](/contributing/architecture.md) says which layer to look in.
