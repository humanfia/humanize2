...
[`/status`](/guide/status). The flow reads and writes it whenever it likes; you change it
...
| | [`/questions`](/guide/questions) | the board |
| --- | --- | --- |
...
- [The shape of a run](/guide/status) — where the board is drawn
- [Questions](/guide/questions) — the half that does stop the turn
- [The person as an agent](/guide/human-agent)
- [Callbacks as tools](/guide/tools) — the other thing that does not stop a turn
