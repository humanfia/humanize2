...
- for **each** flow that workspace has run: what each of its **agents** was running (an agent
  is a CLI the flow runs), where its turns landed, which [account](/guide/providers) it ran as
  and [what it may do](/guide/permissions);
...
`enable_sentry`. It is the answer to the [reporting](/guide/reporting) question, which is asked
...
| Page | |
| --- | --- |
| **Everywhere** | whether humanize [reports what goes wrong](/guide/reporting), and a row that says what a report carries and what it never does |
...
| `~/.humanize/providers/<cli>/<name>/models.json` | the same, as that [account](/guide/providers) |
...
as [traced](/guide/tracing). This is the **profile** row on the second page of `/settings`, and
...
[Tracing › Profiling a run](/guide/tracing#profiling-a-run).
...
and whether [reporting](/guide/reporting) was answered yes. A flow that says it [can be picked
up](/guide/resuming) is handed what the last run of it here left behind, which is the run's own
...
- [History](/guide/history) — the other thing kept between sessions
- [Tracing](/guide/tracing) — the trace a profiled run is drawn into
