...
from the CLI's own. Use one when two agents need to run the same CLI as two different accounts
at once.

An agent with no provider runs its CLI exactly as you run it yourself.
...
at the terminal, and a secret is not echoed — here, the token.
...
`claude` on this machine runs as whoever is signed in there. A flow that wants two of them on
...
to one backend and what signs in to Claude Code is not what signs in to codex. Opening it lists
...
accounts. **a** makes one without leaving the question. It runs the same walk `/providers`
runs, minus the question the `cli` row has already answered, and comes back with the new
account chosen.

## What moves, and what does not

**Only the credential files move.** Sessions, settings and skills stay in the CLI's own home. A
turn under a provider still shows up in a [trace](/guide/tracing), still counts towards the
[cost readout](/guide/tally), and still has the skills you installed.
...
CLI writes them: a login run for a provider is the CLI's own login with those paths pointed
here.
...
credential file it was signed in with. The turn would be taken as the wrong account with
...
The full table, with what each way asks for, is in [Providers › The ways
...
The models an account can run belong to that account. The CLI is asked as soon as one is made,
because which models a turn may name depends on which subscription, key or gateway it runs
under.
...
A CLI that will not say does not fail the line; the account was made. **r** on the models sheet
asks it again, and it is where you find out that the model you came for is not in the list.
...
**enter** opens a menu of four rather than one letter apiece on the list. `l`, `f` and `t` were
three keys to read off the bottom of the screen, and enter, which every list already means, did
one of the four:
...
The other three are held with the removals until the menu is saved, as on every other menu:
correcting one, saying where it falls back to and saying how it is tried again.

The account this machine is already signed into is `as local`, last under each CLI. It is
offered only the bottom two.

The line under them says why rather than leaving two rows that do nothing: humanize did not
make that account and keeps no credentials for it, so there is nothing to correct and nothing
to sign in.
...
ticked and the rest listed and off.
...
those variables, and variables of your own where it has none. So `claude/shared` made by `key`
is `pi/shared` and `opencode/shared` made by `env`: the same key under three names.

So a rotated key is a key rotated in several places at once. The new key is typed once, into
the account it was first made on, and the question after it writes it over the copies that are
**ticked**.

What is ticked is the backends **installed here**. It is the same question as when the account
was made, asked of the account as corrected, and it does not read which backends already hold a
copy. A copy on a CLI that is not on this machine is therefore one still holding the old key
unless somebody turns its row on, and nothing marks it as one: what each row says of its
backend is whether it is installed here.
...
- [Publish a flowverse](/guide/flowverses)
- [A container of its own](/guide/containers)
