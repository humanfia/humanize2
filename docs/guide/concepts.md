...
that do reach you are listed in [Agents](/reference/agents.md#what-each-backend-can-do).
...
variant of the model. See [Agents](/reference/agents.md#efforts).
...
[Agents](/reference/agents.md#the-directory-a-session-works-in).
...
- **can be talked to** — a line said while a turn is running goes *into* that turn rather than
  starting another;
- **can be hooked** — it passes through named [moments](/reference/agents.md#hooks) a flow may hang a
...
flow drives [many turns at once](/reference/flows.md#a-flow-that-waits-for-more-than-one-thing); starting
one is the same either way. It may also [call another flow](/reference/flows.md#a-flow-that-calls-another-flow)
...
configures. See [Machines](/reference/machines.md).
...
neither able to read the other's. See [Providers](/reference/providers.md).
...
Next: [Flows](/reference/flows.md) to write one, [Agents](/reference/agents.md) for the Python API,
[TUI](/reference/tui.md) or [CLI](/reference/cli.md) to look something up.
