...
rather than by directory, so a flow that ran on a [machine of its own](/guide/remote-execution)
...
page of [`/settings`](/guide/settings), which is the page for this directory.
...
A flow that ran on a [machine of its own](/guide/containers) worked in a mirror rather than in
this directory. Find its trajectories with `--session` rather than by workspace.

## Watching instead

A trace is for after. While a run is going, [`/status`](/guide/status) shows the same shape
live. It is read off the turns going past, never by asking the flow.

## See also

- [Tutorial: read the run back](/guide/tracing)
- [Picking a run up](/guide/resuming) — carrying one of these runs on where it stopped
