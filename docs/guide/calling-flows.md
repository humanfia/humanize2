...
about them stays true. Your `builder` is still `builder` in the [trace](/guide/tracing),
whatever the called flow calls the place it filled.

## Pass settings through

A flow that takes [settings of its own](/guide/flow-settings) takes them here too, as a third
...
You may hand a flow that drives [the person](/guide/human-agent) one fewer agent. Nobody
...
- [Hooks](/guide/hooks) — getting between an agent and its turn.
