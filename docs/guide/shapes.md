...
[moving the effort](/guide/efforts) does.
...
Given a schema, [the person](/guide/human-agent) is not shown a JSON Schema. They are asked a
...
Each question goes the road [a coding agent's own question](/guide/questions) takes, so it is a
real question in the interface, options and all. [`/afk`](/guide/afk) or a command line answers
