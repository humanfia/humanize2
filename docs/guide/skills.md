...
Run this to see the skills `claude` loads on this machine:
...
You see the list of skills `claude` would load: yours, and this project's.

## The CLI's own skills

A skill you installed loads for every agent of that CLI. It switches off where that CLI
switches one off, and it is not a setting of any agent. humanize shows you the list:
...
nothing either. What a person has installed is not something a flow is entitled to rewrite. A
list that could be adjusted here while the CLI's own list said otherwise would be two answers
to one question.

## The flow's own skills

A **flow** is a directory. The `skills/` inside it is what that flow works by. It uses the same
layout every one of these CLIs already reads a skill in:
...
See [Flows › The skills a flow brings](/reference/flows#the-skills-a-flow-brings).

The flow's agents get the skills in every session they open. The skills are **mounted**: copied
where that backend reads a project's own skills for as long as the session lives, then taken
away again after. Nothing is installed, and nothing of yours is touched.
...
done with it. A flow called by another flow follows the same rule: whatever is already there
under that name is what both of them read.

They are mounted into the workspace on this machine. An agent [whose turns land
elsewhere](/guide/remote-execution) is given them where that machine reads this directory. A
...
To change what a flow brings, change the flow. Press `f` on it in `/flow` to copy it into
`.humanize/flows/`, skills and all. From then on that name means your copy.

## See also

- [Flows › The skills a flow brings](/reference/flows#the-skills-a-flow-brings)
- [Permissions](/guide/permissions) — a per-agent narrowing that *is* one
