...
the words you [put into one already running](/guide/steering). Both are things you wrote, so
...
list](/guide/completion), the keys move within the list instead.
...
- **Not a session log.** For what actually happened, [`/export`](/guide/export) writes the
  transcript and [`hmz trace collect`](/guide/tracing) writes the whole run.
...
- [What a project remembers](/guide/settings), the other thing kept between sessions
