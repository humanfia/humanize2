...
- [Rules that are checked](#rules-that-are-checked)
...
`tracing/readers/`, its state paths in `coganchor/statepaths.py`, and an entry in `runner.py`'s
class map. Subclass `CommandSessionBase` if a turn is one run of a command line, or
`StreamSessionBase` if it is one long-lived process spoken to a line at a time —
`agents/SPEC.md` says which and why.

**A machine.** Two classes in `machines/`, per [Machines](machines.md#writing-a-machine-of-your-own).
