...
It works whether or not a [flow](flows.md) drove them — a trace of yesterday's `claude` session
is one command away.
...
- [Cycles](#cycles)
- [Where the trajectories come from](#where-the-trajectories-come-from)
- [Narrowing what is collected](#narrowing-what-is-collected)
- [From Python](#from-python)
- [Watching a run instead](#watching-a-run-instead)
...
Full syntax in the [CLI reference](cli.md#hmz-collect).
...
| In the trace | Is |
| --- | --- |
| a **process** | one [agent](concepts.md#agent) |
| a **track** | one [session](concepts.md#session). Sessions of one agent that never run at the same time share a track; root sessions and sub-agents stay apart. |
...
**A flow that ran on a [machine of its own](machines.md) worked in a mirror rather than in this
