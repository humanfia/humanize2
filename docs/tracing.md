...
`hmz collect` reads the trajectories a coding agent recorded, whether or not a
...
Collects the trajectories recorded for a workspace and writes `.humanize/<datetime>.trace.json`.
...
```sh
hmz collect                                   # current workspace, all history
hmz collect ~/myproject --start "3 days ago"  # another workspace, recent history only
...
`hmz collect` takes that from the last run in the workspace rather than being told it. Every
run of a flow is one **cycle**, written to `~/.humanize/cycles/<workspace>/<datetime>-<id>.jsonl`
