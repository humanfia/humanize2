...
nobody is there to give is a flow that has stopped. Anything tighter is a choice, and it is
made on the agent line or in the flow that declares the agent — not at the prompt, where an
agent is a CLI, an account, a model and an effort and nothing else.
