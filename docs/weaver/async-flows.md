...
returns. The agent count, the settings, the [epic](/user/tracing#what-a-run-writes-down) and
...
An agent [stopped](/user/stopping) mid-batch raises `Stopped`, which `suppress` deliberately
does not catch.
...
in the [trace](/user/tracing). See [Many conversations at once](/user/conversations).

## See also

- [Many conversations at once](/user/conversations) for the editor view of every conversation
  that is working.
- [Worktrees](/weaver/worktrees) for rooting each conversation where you want it.
- The [trace](/user/tracing) keeps everything a run writes down.
- An agent [stopped](/user/stopping) mid-batch raises `Stopped`.
