...
run's **[epic](/user/tracing#what-a-run-writes-down)** (what the run writes down) already
...
than on the way out. The question is still worth answering, and [stopping](/user/stopping) is what
stops a flow.
...
Each agent starts at the backend, model, effort, [permission](/user/permissions) and
[account](/user/providers) the run wrote down.
