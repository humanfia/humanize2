...
only**, so a flow that has stopped reads as stopped. [`/status`](/user/status) is the fuller
...
has nothing to read a running cost out of, and [`hmz trace collect`](/user/tracing) has
nothing to gather.
