...
humanize comes with eight: `chat`, `ralph_loop`, `stateful_ralph`, `continue_loop`, `goal`,
`flame_chase`, `rlar` and `humanize1`. Six of them are flowbench's loops written this way. Each
is described in [Flows](flows.md#the-flows-humanize-comes-with).
