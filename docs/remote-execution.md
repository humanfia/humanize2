...
- [Targets](#targets)
- [What the agent observes](#what-the-agent-observes)
- [What reaches the target](#what-reaches-the-target)
- [What stays on this machine](#what-stays-on-this-machine)
- [Anchoring a flow](#anchoring-a-flow)
...
Every flag is in the [CLI reference](cli.md#hmz-anchor).
...
Give an agent's config an anchored [machine](machines.md) and its turns land there, without any
...
[CLI reference](cli.md#hmz-anchor): `target`, `workspace`, `remote_path`, `shadow`,
`local_paths`, `local_execs`, `net`, `net_allow`, `token`, `force`.
