...
- [One CLI, two accounts](#one-cli-two-accounts)
- [Where the credentials are kept](#where-the-credentials-are-kept)
- [The ways in](#the-ways-in)
- [`hmz providers`](#hmz-providers)
- [Choosing one for an agent](#choosing-one-for-an-agent)
- [What a turn under one runs as](#what-a-turn-under-one-runs-as)
- [What a turn under one is run without](#what-a-turn-under-one-is-run-without)
...
home, which is why a turn under a provider still shows up in a [trace](tracing.md), still counts
towards the [cost readout](agents.md#what-it-has-cost-and-how-fast), and still has the skills you
installed.
...
It is a setting of the [agent](agents.md), because it is the agent that signs in.
...
[`hmz anchor`](remote-execution.md) runs a whole session under, here handling only the handful of
syscalls that name one of those files. Everything else the agent does is untouched and runs at
native speed, and the agent is told none of it. That supervisor is [`hmz cred`](cli.md#hmz-cred),
...
- **A turn that is also [anchored](machines.md) is supervised once, not twice.** A process has
...
- **A 32-bit process below the agent is not intercepted**, as it is not under an
  [anchor](remote-execution.md): the filter passes another architecture's syscalls through
...
[machine](machines.md) is configured, so a provider's files do not cross to a target — what
