...
def _watched(agent: AgentBase) -> list[str]:
    ...
    narrated: list[str] = []
    agent.watch(
        lambda _agent, _session, event: narrated.append(event.text)
...
def test_a_backend_that_names_the_kind_itself_is_believed(accounts: None) -> None:
    ...
    failed = Failed(1, ["claude"], "", "some sentence nothing recognises", fault="killed")
...
def test_a_model_that_is_gone_walks_no_account_of_that_cli(
    ...
    assert narrated == []  # nowhere to carry on to, so nothing to narrate carrying on with
...
def test_a_dropped_connection_reopens_the_transport_and_resumes_the_conversation(
    ...
    class ShellAgent(_Shell):  # noqa: N801  -- shadowed on purpose, for the name
...
def test_a_directory_that_has_gone_is_not_a_cli_that_is_not_installed(
    ...
    assert read.fault == "missing"  # a path that is not there and is not where the turn runs
    ...
    assert read.fault == ""  # a failed turn nothing classified, tried again as any other is
...
def test_a_suppressed_turn_still_says_which_kind_it_was(
    ...
    assert agent.new()(_SAYING.format(at=tally, said="401 unauthorized"), suppress=True) == ""
