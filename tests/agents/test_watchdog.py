...
class _Refuses(_Stream):
    """The same, but with a backend that has nowhere to take the asking."""
...
def test_a_session_that_refuses_the_asking_has_its_transport_put_down_instead() -> None:
    """The gentle rung is a rung, not the ladder: a backend that will not take it is not left.

    Every session can be asked -- that is what a turn's budget reaches for when it has spent
    what it was given -- but a backend may still have nowhere to put the question. A window
    spent waiting for something that was never sent is a window a person spends watching
    nothing happen, so the refusal moves the ladder on rather than ending it.
    """
    agent = _agent(_Refuses)
    said = _watched(agent)
    session = agent.new()
    assert isinstance(session, _Refuses)

    with pytest.raises(subprocess.CalledProcessError):
        list(session._stream("go"))

    assert "cannot be interrupted, so its transport goes instead" in _steps(said)
    assert all(proc.returncode is not None for proc in session.started)
