...
def test_the_relay_carries_one_call_to_the_flow_and_the_answer_back(
    ...
    serving.on(
        Moment.PRE_TOOL_USE,
        lambda one: Verdict(refused=True, because="no rm here")
        if "rm " in one.about
        else None,
    )

    def run(command: str, *, pretty: bool = False) -> subprocess.CompletedProcess[bytes]:
...
def test_one_agent_s_moments_are_put_to_a_hook_one_at_a_time(serving: Hooks) -> None:
    ...
    calls = [
        subprocess.Popen(  # noqa: S603 -- our own relay, with our own socket
            serving.gate().command(),
...
def test_claude_is_told_where_this_agent_s_moments_are_on_its_own_command_line() -> None:
...
def test_two_agents_are_two_gates_and_two_settings_files() -> None:
    ...
    theirs = [
        agent.new()._environment()[_SETTINGS]
...
def test_the_wait_a_hook_is_given_is_as_long_as_a_turn_may_be_quiet() -> None:
    ...
    assert WAITING >= _profile("claude").silence
...
def test_a_tool_input_that_is_not_an_object_is_still_answered() -> None:
    """A CLI writes the input as whatever the tool takes, and a gate that died reading one
    would answer nothing at all -- which is the tool going ahead unrefused."""
    hooks = _hooks()
