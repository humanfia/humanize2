...
class Failed(subprocess.CalledProcessError):
    ...
    Which kind it was is worked out where the backend, the exit status and the CLI's own log are all
    to hand, which is `hmz.coganchor.backends.trouble` reached from the session -- not here: a value
    carries what it was told, and reading a message to guess at one is somebody else's job. What is
    written down here is what was decided.
    ...
    Attributes:
      fault: Which kind of failure this was, as `hmz.coganchor.backends.FAULTS` names them, or ""
      for
