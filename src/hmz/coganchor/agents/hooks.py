...
Separate from the base classes for the reason :mod:`hmz.coganchor.agents.event` is: these are the
...
class Moment(StrEnum):
    ...
    :attr:`~hmz.coganchor.agents.base.AgentBase.moments`, and a flow says which it needs where it
    ...
    #: stopping it. `hmz.coganchor.backends` says which backends are which.
...
class Verdict:
    ...
    Attributes:
      refused: Whether what was about to happen may not: the turn does not run, the tool does
        ...
        which is what :attr:`~hmz.coganchor.agents.base.AgentBase.moments` is for -- a hook that
...
#: looks again at whether it has been closed, as :mod:`hmz.coganchor.agents.tools` does it.
_LOOKING = 0.2
...
def answers(line: str, hooks: Hooks) -> str:
    ...
    :func:`hmz.coganchor.agents.tools.serve` is written, for the same reason.
