...
class PiSession(StreamSessionBase):
    ...
    def _environment(self) -> Mapping[str, str]:
        ...
        Returns:
          ...
          theirs is the cache they meant -- and is not set at all for a turn that lands somewhere
          else, since the path would name a directory on this machine rather than on the one the
          turn runs on. The preload is there for an agent something is listening to and absent
          otherwise, which :mod:`hmz.coganchor.agents.preload` decides: pi runs on Node, so what a
          turn of it runs, reads, writes and opens can be read from inside the process taking it.
        """
