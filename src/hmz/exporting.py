    ...
    *,
    ...
    try:
        with tarfile.open(beside, "w:gz") as writing:
            ...
            said = plain(
                json.dumps(
                    _manifest(epic, ran, _sessions(ran, found, _whose(epic)), held),
                    ensure_ascii=False,
                    indent=2,
                ),
                struck,
...
def _sessions(
    ran: Ran, found: Mapping[str, Mapping[str, Path]], whose: Mapping[str, str]
) -> list[dict[str, Any]]:
    ...
    Args:
      ...
      whose: Which record each was opened in, so that two concurrent calls of one flow read
        as the two conversations they were.
    ...
    for one in ran.sessions:
        said: dict[str, Any] = {
            ...
            "session": one.ident,
            "flow": one.flow,
            "record": whose.get(one.name, JOURNAL),
            "at": one.at,
...
def _lines(at: Path) -> list[dict[str, Any]]:
    """Every line one record holds, in the order it was written.

    Args:
      at: The record file.

    Returns:
      One apiece, less whatever cannot be read as one -- a run that died mid-line left a line
      rather than a record, and the rest of it is still what happened.
    """
    held: list[dict[str, Any]] = []
    try:
        said = at.read_text(encoding="utf-8", errors="replace").splitlines()
    except OSError:
        return held
    for line in said:
        with contextlib.suppress(ValueError):
            ...
            if isinstance(one, dict):
                held.append(cast("dict[str, Any]", one))
    return held


def _calls(epic: Path) -> list[dict[str, Any]]:
    """Every flow this run called, at whatever depth, and which record called each.

    Read off the records themselves rather than off the run's own. A flow that called a flow
    that called a flow writes each of those in a record of its own, and the run's own record
    says only what it called directly -- so a manifest built from that alone would be a
    manifest of the first branch of a tree. Each record's opening line says which record
    called it, which is the whole tree with the branches written as links.

    Args:
      epic: The run, by the directory it is written in.

    Returns:
      One entry per called flow, by record name. The tree, flattened: `under` is the parent.
    """
    held: list[dict[str, Any]] = []
    for at in records(epic):
        if at.name == JOURNAL:
            continue  # the run's own record is the flow nothing called
        said = _lines(at)
        nothing: dict[str, Any] = {}
        began = next((one for one in said if one.get("event") == "began"), nothing)
        ended = next(
            (one for one in reversed(said) if one.get("event") == "ended"), nothing
        )
        held.append(
            {
                ...
                "record": at.name,
                # Which record called this one. A flow called twice is two records, so this
                # says which call rather than which flow -- which is what tells two
                # concurrent calls of one flow apart.
                "under": str(began.get("under") or JOURNAL),
                "began": str(began.get("at") or ""),
                "ended": str(ended.get("at") or ""),
                "how": str(ended.get("how") or "") or "left unfinished",
            }
        )
    return held


def _whose(epic: Path) -> dict[str, str]:
    """Which record each session of one run was opened in, by the name the run gave it.

    Which flow opened a session is not enough once a flow calls itself, or calls two of
    something at once: those are two records and two conversations, and a bundle that said
    only the flow would leave a reader unable to tell which of them a log belongs to.

    Args:
      epic: The run, by the directory it is written in.

    Returns:
      One entry per session, saying the record it was written in.
    """
    held: dict[str, str] = {}
    for at in records(epic):
        for said in _lines(at):
            if said.get("event") == "opened" and said.get("name"):
                held[str(said["name"])] = at.name
    return held
