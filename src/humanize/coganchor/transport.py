"""Getting a :class:`~humanize.coganchor.proto.Channel` to ``serve`` on the target.
...
from humanize import coganchor
...
def build_bundle(destination: Path | None = None) -> Path:
    ...
    the target only ever runs ``anchor serve``, which :func:`humanize.cli.main` reaches
    ...
    with tempfile.TemporaryDirectory(prefix="humanize-bundle-") as staging:
        ...
        for depth in range(1, len(parts)):
            # A namespace of its own rather than the installed ``humanize/__init__.py``: the
            # bundle stays pure stdlib however the rest of humanize grows, and a regular package
            # cannot be shadowed by an unrelated ``humanize`` already on the target's path.
