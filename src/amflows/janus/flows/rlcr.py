"""RLCR (PolyArch/humanize) -- an idea is opened, planned against review, then built against it.

/humanize:gen-idea "$IDEA"      # fan a thought into directions, pick one on the evidence
/humanize:gen-plan              # Codex analyses, Claude writes, the two converge
/humanize:start-rlcr-loop       # build it under a Codex that reviews the claim, then the code

    amflows run -f rlcr \
        -a claude/claude-opus-4-8/max,codex/gpt-5.6-sol/max "add undo/redo to the editor"
...
Three commands of a Claude Code plugin, as one run: they are phases here, not commands. What
carried them there -- slash commands, five validator hooks, a state machine in bash -- is what
Claude Code needs to hold a loop together across turns of one conversation, and what a flow, being
one program, does not.

Humanize stops two ways and neither is here: it quizzes you on the plan before it will start, and
it gives up once rounds stop advancing. What the quiz defended is defended instead by a reviewer
that has to accept the plan before a line is built; what the giving up defended is that a reviewer
can be wrong, so each side is told to say where it thinks the other is, rather than to submit.

What is left is the shape that does the work: **the builder remembers and the reviewer does not**.
One session builds, holding the whole run from the first reading of the idea; every review is a
session that has just started, reads the repository itself, and is told nothing about how the work
was arrived at. The only thing read out of a review is whether it is the word COMPLETE; anything
else is the builder's next prompt, word for word, so what the reviewer noticed is what it hears.
"""
...
#: Where the run keeps what outlives a turn: the plan both agents work from, and the tracker the
#: builder keeps of its progress. Named for the agents rather than opened here -- an agent may be
#: working on another machine, where this directory is the only one that exists.
PLAN = ".amflows/rlcr/plan.md"
TRACKER = ".amflows/rlcr/goal-tracker.md"
...
#: otherwise end the run, where a review that meant to accept and said more than the word only
#: costs a round.
ACCEPTED = re.compile(r"\W*COMPLETE\W*", re.IGNORECASE)

#: How every review ends, so that the difference between the three of them is the question asked
#: rather than the wording of the answer.
...
def spoken(session: SessionBase, prompt: str) -> str:
    ...
    """
    while True:
        # `RuntimeError` as well as flowbench's `|| true`: a streaming backend whose process died
        # says so that way, and it is the builder -- the half holding the whole run -- that runs
        # on one. `Stopped` is neither, so the flow still ends when it is ended.
        with suppress(subprocess.CalledProcessError, RuntimeError):
...
def run(agents: tuple[AgentBase, AgentBase], task: str) -> None:
    ...
    building = builder.launch()  # one session for the whole run: the builder remembers
    ...
    told = WRITE_PLAN + spoken(reviewer.launch(), ANALYSE + chosen)
    while True:
        spoken(building, told)
        told = spoken(reviewer.launch(), CHALLENGE + chosen)
    ...
    while True:
        ...
        told = spoken(
            reviewer.launch(), f"{REVIEW}{base}\n\nIts summary of the round:\n{said}"
        )
        if ACCEPTED.fullmatch(told):
            break

    # The claim is settled and is not asked again: a code review that sends the work back would
    # otherwise put it to a reviewer that was not there for the answer, and two reviewers that
    # each remember nothing can go on undoing one another for as long as they are both asked.
    while True:
        told = spoken(
            reviewer.launch(),
