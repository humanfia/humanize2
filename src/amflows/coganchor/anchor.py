...
if TYPE_CHECKING:
    from amflows.coganchor.transport import Target
...
class AnchorConfig:
    ...
    def command(self, argv: Sequence[str]) -> list[str]:
        ...
        interpreter is named explicitly, so the child is the one amflows is installed in
...
def check(config: AnchorConfig | None = None) -> dict[str, Any]:
    ...
    """
    from amflows.coganchor import transport
...
def connect(command: Sequence[str], config: AnchorConfig | None = None) -> int:
    ...
    from amflows.coganchor import __version__, agents, transport
